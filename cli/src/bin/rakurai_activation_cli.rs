...
fn process_init_config(
    ...
    let mut ix = initialize_ix(
        ...
        InitializeArgs {
            ...
            client_commission_bps,
            client_commission_account: AnchorPubkey::new_from_array(
                client_commission_account.as_array().clone(),
            ),
            client_authority: AnchorPubkey::new_from_array(client_authority.as_array().clone()),
            bump,
        },
        InitializeAccounts {
...
fn process_init_pda(
    ...
    let (activation_pubkey, bump) = derive_activation_account_address(
        &program_id,
        &AnchorPubkey::new_from_array(node_pubkey.as_array().clone()),
    );
