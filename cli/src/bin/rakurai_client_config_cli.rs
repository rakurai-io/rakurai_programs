...
fn run_global(
    ...
    match cmd {
        ...
        GlobalCmd::MigrateToV3 => {
            let ix = to_solana_instruction(migrate_global_to_v3_ix(
                program_id_anchor,
                UpdateGlobalAccounts {
                    manager: to_anchor(manager),
                    global: to_anchor(global),
...
fn run_validator(
    ...
    match cmd {
        ...
        ValidatorCmd::MigrateToV3(a) => {
            let validator = validator_pda(program_id_anchor, a.vote);
            let ix = to_solana_instruction(migrate_validator_to_v3_ix(
                program_id_anchor,
                UpdateValidatorAccounts {
                    manager: to_anchor(manager),
                    vote: to_anchor(a.vote),
...
fn run_proposal(
    ...
    match cmd {
        ...
        ProposalCmd::MigrateToV3(a) => {
            ...
            let validator = validator_pda(program_id_anchor, a.vote);
            let proposal = proposal_pda(program_id_anchor, a.vote);
            let ix = to_solana_instruction(migrate_proposal_to_v3_ix(
                program_id_anchor,
                UpdateProposalAccounts {
                    operator: to_anchor(operator),
                    vote: to_anchor(a.vote),
